# -*- coding: utf-8 -*-
"""
finetune_v7.py — 청각장애인 가정용 소리 감지 모델 (YAMNet 전이학습 v7)

v7 변경점: **낙상 클래스 추가** (프로젝트 재정의: 기존 보조기기가 못 잡는 위험 감지)
  - SAFE 데이터셋(Kaggle antonygarciag/fall-audio-detection-dataset, 그래플링 더미 낙상):
    파일명 3번째 필드 00 = 낙상 475개 → 클래스 5, 01~07 = 비낙상 생활음 475개 → 배경(2)
    ※ 비낙상 쿵 소리를 배경에 넣어야 "쿵 = 낙상" 오학습을 막음 (하드 네거티브)
  - 녹음 설정(파일명 1번째 필드 01~10) 단위 그룹 분리: 01~08 train / 09~10 test
  - 추가 목표: 낙상 recall ≥ 0.90 (낙상은 놓치는 것이 최악)
...
달성 성능(테스트 754개, 누수 없음): 정확도 92.7%, 울음 F1 0.978, 유리 F1 1.000, 문 P 1.000, 낙상 R 0.929.
실시간 감지는 realtime_detect_v7.py 사용 (YAMNet이 scores와 embeddings를 함께 출력).
"""
...
SR = 16000
LABELS = ["아기 울음", "유리 깨짐", "일반 소리", "문 소리", "발소리", "낙상"]
NUM_CLASSES = 6
...
FALL_DIR = os.path.join(ROOT, "fall-dataset")
...
MLP_PATHS = [os.path.join(ROOT, f"sound_classifier_v7_mlp{s}.keras") for s in MODEL_SEEDS]
HGB_PATH = os.path.join(ROOT, "sound_classifier_v7_hgb.pkl")
CONFIG_PATH = os.path.join(ROOT, "sound_classifier_v7_config.json")

TESS_SAMPLE = 400          # TESS에서 샘플링할 파일 수
# 클래스별 train 증강 복제 수 (배경음은 증강 없음).
# 낙상 6종 풀세트가 핵심: 게인 0.3x(조용한 낙상)·배경 믹싱 증강으로 recall 0.85→0.93 달성.
AUG_COPIES = {0: 1, 1: 6, 3: 5, 4: 6, 5: 6}
...
def collect_files():
    ...
    # --- SAFE 낙상 데이터셋 (Kaggle, 950개) ---
    # 파일명: {설정}-{id}-{클래스}-{take}-{채널}.wav → 3번째 필드 00 = 낙상, 그 외 = 비낙상 생활음.
    # 비낙상은 배경(2)으로 넣어 "일상 쿵 소리 ≠ 낙상" 경계를 학습시킴.
    # 녹음 설정(1번째 필드) 09~10 = test → 같은 설정의 녹음이 train/test에 겹치지 않게.
    if os.path.isdir(FALL_DIR):
        for f in sorted(os.listdir(FALL_DIR)):
            if not f.lower().endswith(".wav"):
                continue
            parts = f.split("-")
            label = 5 if parts[2] == "00" else 2
            item = (os.path.join(FALL_DIR, f), label)
            (test if parts[0] in ("09", "10") else train).append(item)
...
def main():
    ...
    # ---- 결정 규칙 (전부 val 기준으로 선택, 테스트셋 미사용) ----
    # τ 유리: val에서 타클래스가 유리로 새는 확률 최댓값 + 0.02 (cap 0.92)
    # τ 문:   후보 중 val 문 precision ≥ 0.98이 되는 가장 작은 값 (recall 최대 보존)
    # 낙상 부스트: 낙상 확률에 곱하는 배율 — val 낙상 recall ≥ 0.92이 되는 최소값
    #   (낙상은 놓치는 것이 최악이므로 오탐을 감수하고 recall을 사는 안전 우선 규칙)
    def decide(probs, tau, boost):
        p = probs.copy()
        p[:, 5] *= boost
    ...
    leak_g = p_val[y_val != 1, 1]
    tau_glass = min(float(np.max(leak_g)) + 0.02, 0.92) if len(leak_g) else 0.5

    tau_door = 0.90
    for cand in sorted({round(float(np.quantile(p_val[y_val != 3, 3], 0.995)) + 0.02, 3),
                        0.80, 0.85, 0.90}):
        yp_v = decide(p_val, {1: tau_glass, 3: cand}, 1.0)
        pr_v, _, _, _ = precision_recall_fscore_support(
            y_val, yp_v, labels=range(NUM_CLASSES), zero_division=0)
        if pr_v[3] >= 0.98:
            tau_door = cand
            break

    # 기준 0.92: 0.93으로 두면 부스트가 2.0까지 올라가 배경→낙상 오탐이 과도해짐(실측)
    fall_boost = 1.5
    for cand in (1.0, 1.2, 1.5, 2.0):
        yp_v = decide(p_val, {1: tau_glass, 3: tau_door}, cand)
        _, rc_v, _, _ = precision_recall_fscore_support(
            y_val, yp_v, labels=range(NUM_CLASSES), zero_division=0)
        if rc_v[5] >= 0.92:
            fall_boost = cand
            break

    tau = {1: tau_glass, 3: tau_door}
    print(f"\n결정 규칙: 유리 τ={tau_glass:.3f}, 문 τ={tau_door:.3f}, 낙상 부스트={fall_boost}")

    # ------------------------------------------------------------ 평가 (앙상블 + 결정 규칙)
    y_pred = decide(p_test, tau, fall_boost)
    ...
    targets = [
        ...
        ("낙상 Recall ≥ 0.90", rec[5] >= 0.90, rec[5]),
        # 정확도 기준 92%: v6(5클래스)의 95%보다 낮은 이유는 테스트셋이 어려워졌기 때문
        # (배경에 SAFE 비낙상 '쿵' 95개 추가 + 낙상 recall을 위해 오탐을 의도적으로 감수).
        # 발표 프레임: "5클래스 96.5% / 낙상 확장 6클래스 92%+ (SAFE 원논문 이진분류 93% 수준)"
        ("전체 정확도 ≥ 92%", acc >= 0.92, acc),
    ]
    print("\n[목표 성능 판정]")
    all_pass = True
    ...
    with open(CONFIG_PATH, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "decision": "avg(mean(MLP3), HGB) -> 낙상 확률 x fall_boost -> argmax "
                        "-> 유리/문 예측이 tau 미만이면 일반 소리로 강등",
